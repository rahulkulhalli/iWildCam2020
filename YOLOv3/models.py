    ...
    def forward(self, x, targets=None, img_dim=None, cls=None):
        ...
        if targets is None:
            ...
            if obj_mask.sum() > 0:
                ...
                loss_cls = self.ce_loss(pred_cls[obj_mask].view(-1, self.num_classes), tcls[obj_mask].long().view(-1))
            ...
            self.metrics['classification_loss'] = to_cpu(_classification_loss).item()
            _weighted_class_scores = weighted_class_scores.clone()
            self.metrics['batch_acc'] = to_cpu(torch.sum(torch.argmax(_weighted_class_scores, dim=-1) == cls)).item()/len(cls)
...
class Darknet(nn.Module):
    ...
    def forward(self, x, targets=None, cls=None):
        ...
        yolo_class_scores = torch.FloatTensor(size=(x.shape[0], self.yolo_layers[0].num_classes)).zero_().to(x.device)
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            if module_def["type"] in ["convolutional", "upsample", "maxpool"]:
                ...
                x, class_scores, layer_loss = module[0](x, targets, img_dim, cls)
                loss += layer_loss
                yolo_outputs.append(x)
                yolo_class_scores += self.softmax(class_scores.detach())
            layer_outputs.append(x)
        yolo_outputs = to_cpu(torch.cat(yolo_outputs, 1))
        yolo_class_scores = to_cpu(yolo_class_scores/3.)
        # yolo_outputs = torch.cat(yolo_outputs, 1)
        return (yolo_outputs, yolo_class_scores) if targets is None else (loss, yolo_outputs, yolo_class_scores)
